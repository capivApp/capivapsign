...
export type UserProfileTeaserProps = {
  className?: string;
  rows?: number;
};

/**
 * Illustrative preview of a public signing profile, shown beside the sign-up
 * form. Deliberately generic: no real person, no real profile — it exists to
 * show the shape of the feature, not to reference anyone.
 */
export const UserProfileTeaser = ({ className, rows = 2 }: UserProfileTeaserProps) => {
  ...
  return (
    <div className={cn('flex flex-col items-center rounded-xl bg-neutral-100 p-4 dark:bg-background', className)}>
      <div className="inline-block max-w-full truncate rounded-md border border-border bg-background px-2.5 py-1.5 text-muted-foreground text-sm">
        {baseUrl.host}/u/seu-perfil
      </div>

      <div className="mt-4">
        <img src={LogoIcon} className="h-20 w-20 rounded-full object-contain" alt="CapivaSign" />
      </div>

      <div className="mt-6">
        <div className="flex items-center justify-center gap-x-2">
          <h2 className="font-semibold text-2xl">
            <Trans>Your public profile</Trans>
          </h2>
        ...
        <p className="mt-4 max-w-[40ch] text-center text-muted-foreground text-sm">
          <Trans>Share one link and let anyone start signing with you.</Trans>
        </p>

...
export type WhatsappTransportDeleteDialogProps = {
  transportId: string;
...
export const WhatsappTransportDeleteDialog = ({
  transportId,
  transportName,
  subscriptionClaimCount,
  organisationClaimCount,
  trigger,
}: WhatsappTransportDeleteDialogProps) => {
  const { t } = useLingui();
  ...
  const { mutateAsync: deleteTransport, isPending } = trpc.admin.whatsappTransport.delete.useMutation({
    onSuccess: () => {
      toast({
  ...
  return (
    <Dialog open={open} onOpenChange={(value) => !isPending && setOpen(value)}>
      ...
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            <Trans>Delete WhatsApp Transport</Trans>
          </DialogTitle>
          <DialogDescription>
        ...
        {isInUse && (
          <Alert variant="destructive">
            <AlertDescription>
              <Trans>Warning, this WhatsApp transport is currently being used by:</Trans>

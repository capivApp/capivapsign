...
## How CapivaSign Supports Compliance

CapivaSign provides features that support compliance with e-signature laws across jurisdictions:
...
CapivaSign maintains an audit trail for each document, recording:
...
CapivaSign supports compliance with Simple Electronic Signature (SES) requirements. The following are not currently provided:

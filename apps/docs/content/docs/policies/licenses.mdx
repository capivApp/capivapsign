---
title: Licenses
description: CapivaSign uses a dual licensing model with an open-source AGPL-3.0 license (Community Edition) and a commercial license (Enterprise Edition).
---
...
- Use CapivaSign for any purpose, including commercial use
- Access, study, and modify the source code
- Distribute copies of the original or modified software
- Self-host CapivaSign on your own infrastructure

### What AGPL-3.0 Requires

The AGPL-3.0 has specific requirements that apply when you modify or distribute CapivaSign:

**Source Code Disclosure**: If you modify CapivaSign and make it available over a network, you must provide access to the complete source code of your modified version under AGPL-3.0.
...
<Callout type="info">
  ...
  network access as distribution. If users interact with your modified CapivaSign instance over a
...
- Use CapivaSign internally without modifications
...
The Enterprise Edition provides a commercial license that removes the AGPL-3.0 requirements. This allows organisations to use, modify, and integrate CapivaSign without source code disclosure obligations.
...
**Proprietary Integration**: Integrate CapivaSign into proprietary products and services without licensing conflicts.
...
- Build commercial products that incorporate CapivaSign
...
<Callout type="warn">
  If you modify CapivaSign and provide network access to users (internal or external) without
...
<Accordions type="multiple">
  <Accordion title="Do you modify CapivaSign?">
    **No modifications**: Both licenses work. Community Edition is free and sufficient if you run CapivaSign as-is.
  ...
  <Accordion title="Are you embedding CapivaSign in a commercial product?">
  ...
  <Accordion title="Internal use only?">
    Using CapivaSign internally without providing network access to external users does not trigger AGPL-3.0 distribution requirements. However, if your organisation has policies against AGPL software or you make modifications you want to keep private, consider the Enterprise Edition.
  </Accordion>
</Accordions>
...
<Accordions type="multiple">
  ...
  <Accordion title="Does using the CapivaSign API require a commercial license?">
    No. Calling the CapivaSign API from your application does not create a derivative work. Your
    application remains under your chosen license. However, if you modify CapivaSign itself and
    provide network access, AGPL-3.0 requirements apply.
  </Accordion>
  <Accordion title='What counts as a "modification" under AGPL-3.0?'>
    Modifying the source code, forking the repository and making changes, or linking CapivaSign
    libraries into your application in a way that creates a combined work. Configuration changes and
    using CapivaSign through its API are not modifications.
  </Accordion>
  <Accordion title="Can I white-label CapivaSign?">
  ...
  <Accordion title="Can I contribute to CapivaSign under the Enterprise license?">
    Contributions to the main CapivaSign repository are licensed under AGPL-3.0. The Enterprise
...
- [GitHub Repository](https://github.com/capivapp/capiva-sign) - Source code and LICENSE file

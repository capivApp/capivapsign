...
When someone uses a direct link, CapivaSign creates a new document from the template with that person as the signer.
...
curl -X POST "https://app.capivapp.com.br/api/v2/template/direct/create" \
...
// URL: https://app.capivapp.com.br/d/{token}
````
</Tab>
</Tabs>
...
```
https://app.capivapp.com.br/d/{token}
```
...
```html
<iframe
  src="https://app.capivapp.com.br/embed/direct/abc123xyz"
  width="100%"
  height="800"
  frameborder="0"
...
function DocumentSigner({ token }: { token: string }) {
  return (
    <div className="h-[800px] w-full">
      <iframe
        src={`https://app.capivapp.com.br/embed/direct/${token}`}
...
function redirectToSigning(token: string) {
  window.location.href = `https://app.capivapp.com.br/d/${token}`;
}
```
...
```
https://app.capivapp.com.br/d/abc123xyz?externalId=order-12345
```
...
function getSigningUrl(token: string, orderId: string) {
  ...
  return `https://app.capivapp.com.br/d/${token}?${params.toString()}`;
}
...
```
https://app.capivapp.com.br/embed/direct/abc123xyz?externalId=order-12345
```
...
| Account   | Signer must be logged into a CapivaSign account |
...
<Tab value="curl">
```bash
curl -X POST "https://app.capivapp.com.br/api/v2/template/direct/toggle" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123,
...
```bash
curl -X POST "https://app.capivapp.com.br/api/v2/template/direct/delete" \
  -H "Authorization: api_xxxxxxxxxxxxxxxx" \
  -H "Content-Type: application/json" \
  -d '{
    "templateId": 123
...
Enterprise plans support white-label embedding with the "Powered by CapivaSign" badge removed. [Contact sales](mailto:support@capivapp.com.br) for details.
...
function getEmbedUrl(token: string, orderId: string) {
  ...
  return `https://app.capivapp.com.br/embed/direct/${token}?${params}`;
}

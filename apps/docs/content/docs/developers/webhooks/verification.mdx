...
Verifying webhook requests ensures that incoming payloads originate from CapivaSign and have not been tampered with. Without verification, attackers could forge requests to your endpoint and trigger unintended actions.

## How CapivaSign Signs Webhooks

When you configure a webhook with a secret, CapivaSign includes that secret in every webhook request via the `X-CapivaSign-Secret` header. Your server should compare this header value against your stored secret to authenticate the request.

```bash
POST /your-webhook-endpoint HTTP/1.1
Host: your-server.com
Content-Type: application/json
X-CapivaSign-Secret: your_webhook_secret_here
...
| Header               | Description                                             |
| -------------------- | ------------------------------------------------------- |
| `X-CapivaSign-Secret` | The secret key you configured when creating the webhook |
...
<Steps>
  <Step>
    Extract the `X-CapivaSign-Secret` header from the incoming request
  </Step>
  <Step>
...
app.post('/webhooks/capivasign', (req, res) => {
  const receivedSecret = req.headers['x-capivasign-secret'];
...
@app.route('/webhooks/capivasign', methods=['POST'])
def handle_webhook():
    received_secret = request.headers.get('X-CapivaSign-Secret', '')
...
| --------------- | --------------------------------------- | ------------------------------------------------------------------ |
| Secret mismatch | Webhook secret changed or misconfigured | Verify the secret in your environment matches the one in CapivaSign |
| Empty header    | Webhook created without a secret        | Add a secret to the webhook configuration in CapivaSign             |
...
3. Update the webhook configuration in CapivaSign
...
app.post('/webhooks/capivasign', webhookLimiter, (req, res) => {
  // Handle webhook...
});
```

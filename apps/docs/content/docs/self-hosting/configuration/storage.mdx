...
S3 storage is recommended for production deployments. CapivaSign supports AWS S3 and any S3-compatible storage service.
...
```bash
aws s3 mb s3://your-capivasign-bucket --region us-east-1
```
...
[
  {
    ...
    "AllowedOrigins": ["https://your-capivasign-domain.com"],
    "ExposeHeaders": ["ETag"]
  }
]
```
...
```bash
aws s3api put-bucket-cors --bucket your-capivasign-bucket --cors-configuration file://cors.json
```
  ...
  "Statement": [
    {
      ...
      "Resource": "arn:aws:s3:::your-capivasign-bucket/*"
    }
  ]
}
```
...
Using Docker Compose with CapivaSign:
...
```bash
NEXT_PUBLIC_UPLOAD_TRANSPORT=s3
NEXT_PRIVATE_UPLOAD_BUCKET=capivasign
...
CapivaSign works with any S3-compatible storage service. Configure the endpoint and enable path-style URLs if required.
...
Azure Blob Storage is supported as a native transport (not S3-compatible). CapivaSign uses the official `@azure/storage-blob` SDK and signs SAS URLs with the Storage Account key for browser uploads and downloads.
...
az storage cors add \
  --services b \
  --methods GET PUT \
  --origins https://your-capivasign-domain.com \
...
NEXT_PRIVATE_UPLOAD_AZURE_CONTAINER=capivasign-documents
```
...
- An S3 bucket configured for CapivaSign
...
CapivaSign does not provide automatic migration between storage backends. Each document's storage location is recorded in the database.
...
During migration, CapivaSign can read from both backends. The `DocumentData.type` field indicates where each document is stored.

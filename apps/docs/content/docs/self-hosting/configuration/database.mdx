...
description: Configure PostgreSQL connection strings, pooling, SSL, migrations, and performance tuning for your CapivaSign deployment.
---
...
CapivaSign requires **PostgreSQL 14 or later**. No other databases are supported.
...
```
postgresql://capivasign:password@localhost:5432/capivasign
```

**Remote server with SSL:**

```
postgresql://capivasign:password@db.example.com:5432/capivasign?sslmode=require
```
...
```
postgresql://capivasign:p%40ss%23word@localhost:5432/capivasign
```

## Environment Variables

CapivaSign uses two database connection variables:
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@host:5432/capivasign
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@host:5432/capivasign
```

### Automatic Detection

CapivaSign automatically detects common database environment variable formats used by hosting providers:
...
If your hosting provider sets these variables, CapivaSign will use them automatically.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@pooler-host:6432/capivasign?pgbouncer=true

# Direct connection (bypasses PgBouncer)
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@db-host:5432/capivasign
```
...
CapivaSign uses Prisma, which maintains its own connection pool. Configure pool size with connection string parameters:

```
postgresql://user:password@host:5432/capivasign?connection_limit=10&pool_timeout=30
```
...
```
postgresql://user:password@host:5432/capivasign?sslmode=require
```
...
```
postgresql://user:password@host:5432/capivasign?sslmode=verify-full&sslrootcert=/path/to/ca.crt
```
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/capivasign?sslmode=verify-full&sslrootcert=/etc/ssl/certs/db-ca.crt" \
  capivasign/capivasign:latest
```

## Running Migrations

Database migrations update your schema when upgrading CapivaSign.

### Automatic Migrations

When running CapivaSign via Docker, migrations run automatically on container startup. No manual intervention is required.
...
<Accordions type="multiple">
  ...
  <Accordion title="Connection timeout during migration">
    ...
    ```bash
    NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@host:5432/capivasign?connect_timeout=60"
    ```
  </Accordion>
</Accordions>
...
<Accordions type="multiple">
  ...
  <Accordion title="Neon">
    ...
    **Configuration:**
    ...
    ```bash
    # Pooled connection
    NEXT_PRIVATE_DATABASE_URL=postgresql://[user]:[password]@[endpoint]-pooler.region.aws.neon.tech/capivasign?sslmode=require

    # Direct connection for migrations
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://[user]:[password]@[endpoint].region.aws.neon.tech/capivasign?sslmode=require
    ```
  ...
  <Accordion title="AWS RDS">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://capivasign:[password]@your-instance.region.rds.amazonaws.com:5432/capivasign?sslmode=require
    NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://capivasign:[password]@your-instance.region.rds.amazonaws.com:5432/capivasign?sslmode=require
    ```
  ...
  <Accordion title="Google Cloud SQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://capivasign:[password]@/capivasign?host=/cloudsql/[project]:[region]:[instance]
    ```
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://capivasign:[password]@[public-ip]:5432/capivasign?sslmode=require
    ```
  ...
  <Accordion title="Azure Database for PostgreSQL">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://capivasign@[server-name]:[password]@[server-name].postgres.database.azure.com:5432/capivasign?sslmode=require
    ```
  ...
  <Accordion title="DigitalOcean Managed Databases">
    **Configuration:**
    ...
    ```bash
    NEXT_PRIVATE_DATABASE_URL=postgresql://doadmin:[password]@[cluster-host]:25060/capivasign?sslmode=require
    ```
...
```bash
pg_dump -h host -U user -d capivasign -F c -f capivasign_backup.dump
```

**Restore from backup:**

```bash
pg_restore -h host -U user -d capivasign -c capivasign_backup.dump
```
...
Key parameters for CapivaSign workloads:
...
CapivaSign includes necessary indexes by default. Additional indexes may help for:
...
<Accordions type="multiple">
  ...
  <Accordion title="Authentication failed">
    ...
    Solutions:
    - Reset password with `ALTER USER capivasign WITH PASSWORD 'newpassword';`
  ...
  <Accordion title="Database does not exist">
    ...
    ```sql
    CREATE DATABASE capivasign;
    GRANT ALL PRIVILEGES ON DATABASE capivasign TO capivasign;
    ```
  </Accordion>
  <Accordion title="Permission denied">
    ...
    ```sql
    GRANT ALL PRIVILEGES ON ALL TABLES IN SCHEMA public TO capivasign;
    GRANT ALL PRIVILEGES ON ALL SEQUENCES IN SCHEMA public TO capivasign;
    ```
  </Accordion>
</Accordions>

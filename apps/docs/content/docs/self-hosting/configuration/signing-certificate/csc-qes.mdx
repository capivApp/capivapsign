...
The `csc` signing transport routes signatures through a third-party Trust Service Provider (TSP) using the [Cloud Signature Consortium API v1.0.4.0](https://cloudsignatureconsortium.org/). Each recipient authenticates directly with the TSP (Strong Customer Authentication) and the TSP returns a per-recipient signature bound to the document hash. CapivaSign assembles the resulting PAdES signature inside the PDF.
...
<Callout type="warn">
  CSC mode is **instance-wide**: one CSC provider per CapivaSign install. All envelopes created
...
Register CapivaSign as an OAuth client with the TSP. You will receive a client ID and client secret, and must supply CapivaSign's callback URL when registering:

```
${NEXT_PUBLIC_WEBAPP_URL}/api/csc/oauth/callback
```

The callback URL is fixed — CapivaSign derives it from `NEXT_PUBLIC_WEBAPP_URL` and the route mount path. There is no env var to override it; ensuring the registered URL matches your instance's webapp URL exactly is the operator's responsibility.
...
NEXT_PRIVATE_SIGNING_CSC_OAUTH_CLIENT_ID=capivasign-prod
...
The signed PDF must remain byte-identical to what each recipient's TSP signature authorised — CapivaSign cannot decorate it after signing. Audit logs and the Certificate of Completion are generated on demand and delivered as separate PDFs:
...
2. CapivaSign redirects to the TSP for Strong Customer Authentication (first visit only; cached for the session lifetime).
...
5. Returns to CapivaSign; the signing call completes within ~15 seconds.
...
CapivaSign refuses TSP credentials that do not meet the following minimums, at the OAuth callback boundary and again at sign time:

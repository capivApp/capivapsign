...
description: Configure the X.509 signing certificate used for digital signatures in self-hosted CapivaSign.
---

import { Tab, Tabs } from 'fumadocs-ui/components/tabs';

When a document is completed in CapivaSign, it is digitally signed using an X.509 certificate. This cryptographic signature provides:
...
Self-hosted CapivaSign instances require a signing certificate. You can generate a self-signed certificate or purchase one from a Certificate Authority (CA).
...
For organisations requiring hardware-based key protection, CapivaSign supports Google Cloud HSM. This provides:
...
For Advanced and Qualified Electronic Signatures under eIDAS, CapivaSign integrates with third-party Trust Service Providers via the Cloud Signature Consortium API. Each recipient authenticates directly with the TSP, which holds the private key and issues the signature.
...
- Instance-wide setting; one CSC provider per CapivaSign install

...
CapivaSign supports four email transport methods:
...
SMTP is the most common email transport. CapivaSign supports two SMTP authentication methods.
...
Generate an API key in the Resend dashboard for CapivaSign to use.
...
CapivaSign uses React Email templates for all outgoing emails. The templates are located in `packages/email/templates/` and include:
...
- Custom logo replaces the CapivaSign logo
...
Configure CapivaSign to use the test server:
...
NEXT_PRIVATE_SMTP_FROM_NAME="CapivaSign Test"
```
...
```bash
# Docker
docker logs capivasign 2>&1 | grep -i email
...
For MailChannels, configure DKIM directly in CapivaSign using the environment variables described above.

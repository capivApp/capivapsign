...
description: Enable DOCX uploads on a self-hosted CapivaSign instance by running a Gotenberg sidecar that converts Word documents to PDF.
---
...
CapivaSign can accept `.docx` uploads in addition to PDFs. When a user uploads a Word document, the CapivaSign server sends it to a [Gotenberg](https://gotenberg.dev) service which uses LibreOffice to convert it to PDF. The converted PDF is what gets stored, signed, and downloaded. The original DOCX is discarded.
...
| Output format           | PDF                                                                  |
| Network requirement     | CapivaSign must reach the Gotenberg HTTP API                          |
...
- Network reachability from the CapivaSign container to the Gotenberg HTTP API.
- A version of CapivaSign that includes the document conversion feature.
...
For better fidelity, especially for non-Latin scripts, build a derived image that adds Microsoft Core Fonts and additional language fonts. The CapivaSign repository ships a reference Dockerfile at [`docker/development/Dockerfile.gotenberg`](https://github.com/capivapp/capiva-sign/blob/main/docker/development/Dockerfile.gotenberg) that you can use as a starting point:
...
```bash
docker build -t registry.example.com/capivasign/gotenberg:8 \
  -f Dockerfile.gotenberg .
docker push registry.example.com/capivasign/gotenberg:8
```
...
The Gotenberg service should run **alongside** your CapivaSign container, not exposed to the public internet. The conversion service has no built-in authorisation beyond HTTP Basic auth, so it should sit on a private network or behind your existing reverse proxy.

<Tabs items={['Docker Compose', 'Kubernetes', 'External Instance']}>
<Tab value="Docker Compose">

Add a `gotenberg` service to the `compose.yml` you use for CapivaSign:

```yaml
services:
  gotenberg:
    image: registry.example.com/capivasign/gotenberg:8
...
Do **not** publish Gotenberg's port (`3000`) to the host. CapivaSign reaches it over the internal Docker network using the service name (`http://gotenberg:3000`).
...
Then reference the in-cluster URL from CapivaSign's environment:

```
NEXT_PRIVATE_DOCUMENT_CONVERSION_URL=http://gotenberg.capivasign.svc.cluster.local:3000
NEXT_PRIVATE_DOCUMENT_CONVERSION_USERNAME=capivasign
...
CapivaSign does not have to colocate with Gotenberg. You can point it at any reachable Gotenberg deployment: a managed instance, a shared internal service, or a Gotenberg-compatible API.
...
- Be reachable from the CapivaSign container with low enough latency that the 30 second per-request timeout is comfortable.
...
| `--api-timeout=500s`              | Server-side request ceiling. CapivaSign aborts at 30 s by default, so this is a safety net for very large documents.                                                                                                                                                  |
...
| `--pdfengines-disable-routes`     | Disables the PDF engines routes CapivaSign does not use. Shrinks the attack surface.                                                                                                                                                                                  |
| `--webhook-disable`               | Disables webhook callbacks. CapivaSign uses synchronous requests only.                                                                                                                                                                                                |

## Configure CapivaSign

Set the following environment variables on the CapivaSign container and restart it.
...
<Steps>
<Step>
### Restart the CapivaSign container
...
From a shell inside the CapivaSign container or another container on the same network:
...
In the CapivaSign web UI, open **Documents** and try uploading a small `.docx` file. The upload dropzone should accept it, and after a few seconds the editor should open with the converted PDF.
...
- **Disable unused routes.** `--pdfengines-disable-routes` and `--webhook-disable` reduce attack surface. CapivaSign only uses the LibreOffice convert route.
...
- **Rotate credentials.** Rotating the basic auth secret is a config change in both Gotenberg and CapivaSign, followed by a restart of each.
...
<Accordions type="multiple">
  <Accordion title="DOCX uploads are rejected with 'Only PDF and DOCX files are allowed'">
    The CapivaSign server does not see `NEXT_PRIVATE_DOCUMENT_CONVERSION_URL`. Check the value is set
    on the running container (`docker exec capivasign printenv | grep DOCUMENT_CONVERSION`) and
    restart after changing it.
  </Accordion>

  <Accordion title="Uploads fail with 'Document conversion service is currently unavailable'">
    CapivaSign could not reach Gotenberg. Verify:

    - The URL in `NEXT_PRIVATE_DOCUMENT_CONVERSION_URL` is resolvable from the CapivaSign container
  ...
  <Accordion title="Form controls in the DOCX appear blank or missing">
    CapivaSign disables Gotenberg's `exportFormFields` flag during conversion. Word content controls
    (`<w:sdt>`) become static graphics in the output PDF, which prevents CapivaSign's later
    flattening step from making them invisible. This is intentional. Use CapivaSign fields

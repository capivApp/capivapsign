---
title: Docker
description: Run CapivaSign as a standalone Docker container with an external database. Ideal for container platforms like AWS ECS, Google Cloud Run, or Azure Container Instances.
---
...
The CapivaSign image is available on both DockerHub and GitHub Container Registry:
...
docker pull ghcr.io/capivapp/capiva-sign:latest
```
...
These variables must be set for CapivaSign to start:
...
| `NEXT_PUBLIC_WEBAPP_URL`                | Public URL where CapivaSign is accessible (e.g., `https://sign.example.com`) | -                             |
...
CapivaSign requires two database connection strings:
...
| `NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH`      | Path to signing certificate inside container                   | `/opt/capivasign/cert.p12` |
...
docker run -d \
  ...
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@db-host:5432/capivasign" \
  ...
  -e NEXT_PRIVATE_SMTP_FROM_NAME="CapivaSign" \
...
- **Volume mount** — mount a `.p12` file from the host into the container at `/opt/capivasign/cert.p12` (shown above). This is the simplest approach for small to moderate deployments.
...
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@db-host:5432/capivasign
...
CapivaSign provides health check endpoints for monitoring:
...
```bash
docker inspect --format='{{.State.Health.Status}}' capivasign
```

## Updating the Container

To update CapivaSign to a new version:
...
The CapivaSign container is stateless. All persistent data is stored in the PostgreSQL database.
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check the container logs: `docker logs capivasign`. Common causes: missing required environment

...
description: Deploy CapivaSign on a Linux server without Docker using Node.js and systemd.
---
...
Navigate to the `capivasign` folder and create a `.env` file:
...
NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@localhost:5432/capivasign"
NEXT_PRIVATE_DIRECT_DATABASE_URL="postgresql://user:password@localhost:5432/capivasign"
NEXT_PRIVATE_SMTP_FROM_NAME="CapivaSign"
NEXT_PRIVATE_SMTP_FROM_ADDRESS="noreply@your-domain.com"
```

<Callout type="info">
  If you use a reverse proxy in front of CapivaSign, set `NEXT_PUBLIC_WEBAPP_URL` to the public URL
...
Create a service file to run CapivaSign as a background service on Linux:

```ini
# /etc/systemd/system/capivasign.service

[Unit]
Description=CapivaSign
After=network.target

[Service]
Type=simple
User=www-data
WorkingDirectory=/var/www/capivasign
EnvironmentFile=/var/www/capivasign/.env
...
```bash
sudo systemctl enable capivasign
sudo systemctl start capivasign
```

## Reverse Proxy with Nginx

A minimal Nginx configuration for proxying to CapivaSign:

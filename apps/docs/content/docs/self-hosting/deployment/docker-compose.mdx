...
description: Deploy CapivaSign with Docker Compose, including PostgreSQL. This production-ready setup is suitable for most self-hosted deployments.
---
...
```bash
mkdir capivasign && cd capivasign
curl -O https://raw.githubusercontent.com/capivasign/capivasign/release/docker/production/compose.yml
```

Alternatively, clone the full repository:

```bash
git clone https://github.com/capivapp/capiva-sign.git
cd capivasign/docker/production
```
...
services:
  ...
  capivasign:
    ...
    ports:
      - ${PORT:-3000}:${PORT:-3000}
    volumes:
      - /opt/capivasign/cert.p12:/opt/capivasign/cert.p12:ro
...
| `capivasign` | Main application container, waits for database to be healthy |

The CapivaSign container waits for the database health check to pass before starting.
...
```bash
# Database (used by both database and capivasign services)
...
# Public URL where CapivaSign is accessible
NEXT_PUBLIC_WEBAPP_URL=https://sign.example.com
NEXT_PRIVATE_INTERNAL_WEBAPP_URL=http://localhost:3000

# Database connection (uses Docker service name)
NEXT_PRIVATE_DATABASE_URL=postgresql://capivasign:your-secure-database-password@database:5432/capivasign
...
```bash
sudo mkdir -p /opt/capivasign
sudo cp /path/to/your/cert.p12 /opt/capivasign/cert.p12
sudo chown 1001:1001 /opt/capivasign/cert.p12
sudo chmod 400 /opt/capivasign/cert.p12
```
...
```
NAME                             STATUS              PORTS
capivasign-production-database-1  running (healthy)   5432/tcp
capivasign-production-capivasign-1 running             0.0.0.0:3000->3000/tcp
```
...
```bash
docker compose --env-file .env restart capivasign
```
...
For production, place CapivaSign behind a reverse proxy for SSL termination:
...
```bash
# Manual backup
docker compose exec database pg_dump -U capivasign capivasign > backup.sql

# Restore from backup
docker compose exec -T database psql -U capivasign capivasign < backup.sql
```
...
```bash
NEXT_PRIVATE_DATABASE_URL=postgresql://user:password@your-db-host:5432/capivasign
NEXT_PRIVATE_DIRECT_DATABASE_URL=postgresql://user:password@your-db-host:5432/capivasign
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start">
    Check logs: `docker compose logs capivasign`. Common causes: missing environment variables
  ...
  <Accordion title="Database connection errors">
    ...
    database psql -U capivasign -d capivasign -c "SELECT 1"`.
  </Accordion>
  ...
  <Accordion title="Emails not sending">
    ...
    password, check logs: `docker compose logs capivasign | grep -i smtp`.
  </Accordion>

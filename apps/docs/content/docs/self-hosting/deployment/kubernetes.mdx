---
title: Kubernetes
description: Deploy CapivaSign on Kubernetes for production environments requiring high availability, auto-scaling, and enterprise-grade infrastructure.
---
...
A typical CapivaSign Kubernetes deployment consists of:
...
Create a dedicated namespace for CapivaSign:
...
metadata:
  name: capivasign
  labels:
    app.kubernetes.io/name: capivasign
```
...
stringData:
  ...
  NEXT_PRIVATE_DATABASE_URL: 'postgresql://user:password@postgres-host:5432/capivasign'
  NEXT_PRIVATE_DIRECT_DATABASE_URL: 'postgresql://user:password@postgres-host:5432/capivasign'
...
data:
  ...
  NEXT_PRIVATE_SMTP_FROM_NAME: 'CapivaSign'
  NEXT_PRIVATE_SMTP_FROM_ADDRESS: 'noreply@example.com'
  NEXT_PRIVATE_INTERNAL_WEBAPP_URL: 'http://localhost:3000'
  NEXT_PRIVATE_SIGNING_LOCAL_FILE_PATH: '/opt/capivasign/cert.p12'
...
Create the CapivaSign Deployment:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: capivasign
          image: capivasign/capivasign:latest
          ...
          volumeMounts:
            - name: signing-cert
              mountPath: /opt/capivasign/cert.p12
              subPath: cert.p12
              readOnly: true
      volumes:
        - name: signing-cert
          secret:
            secretName: capivasign-signing-cert
            items:
              - key: cert.p12
                path: cert.p12
```
...
<Callout type="info">
  Pin to a specific image tag (e.g., `capivasign/capivasign:1.5.0`) in production instead of `latest`
...
```bash
kubectl create secret generic capivasign-signing-cert \
  --namespace capivasign \
...
type: Opaque
stringData:
  POSTGRES_USER: capivasign
  POSTGRES_PASSWORD: your-secure-password
  POSTGRES_DB: capivasign
---
apiVersion: apps/v1
kind: Deployment
metadata:
...
Update your CapivaSign secret to use the in-cluster database:

```yaml
NEXT_PRIVATE_DATABASE_URL: 'postgresql://capivasign:your-secure-password@postgres.capivasign.svc.cluster.local:5432/capivasign'
NEXT_PRIVATE_DIRECT_DATABASE_URL: 'postgresql://capivasign:your-secure-password@postgres.capivasign.svc.cluster.local:5432/capivasign'
```

## Persistent Storage

CapivaSign stores documents in the database by default. For high-volume deployments, configure S3-compatible storage.
...
stringData:
  ...
  NEXT_PRIVATE_UPLOAD_BUCKET: 'your-capivasign-bucket'
...
Scale CapivaSign based on CPU and memory utilization:
...
<Accordions type="multiple">
  <Accordion title="Pods not starting">
    ...
    ```bash
    # Get pod status
    kubectl get pods -n capivasign
    
    # Describe pod
    kubectl describe pod <pod-name> -n capivasign
    ```
  ...
  <Accordion title="Database connection errors">
    ...
    ```bash
    kubectl run postgres-test --rm -it --image=postgres:15-alpine -n capivasign -- psql "postgresql://user:password@host:5432/database" -c "SELECT 1"
    ```
  </Accordion>
  <Accordion title="Ingress not working">
    ...
    ```bash
    # Get Ingress status
    kubectl get ingress -n capivasign
    
    # Describe Ingress
    kubectl describe ingress capivasign -n capivasign
    ```
  ...
  <Accordion title="Certificate errors">
    ...
    kubectl get secret capivasign-signing-cert -n capivasign -o yaml
    ```
    ...
    kubectl exec -it <pod-name> -n capivasign -- ls -la /opt/capivasign/
    ```
  </Accordion>

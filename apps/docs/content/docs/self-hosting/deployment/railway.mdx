---
title: Railway
description: Deploy CapivaSign on Railway with automatic builds from Git, managed PostgreSQL, and built-in networking. Use the one-click template or manual setup.
---
...
<Callout type="warn">
  ...
  [GitHub Discussions](https://github.com/capivapp/capiva-sign/discussions) for help.
</Callout>
...
The fastest way to deploy CapivaSign on Railway is using the official template:
...
- CapivaSign application service
- PostgreSQL database
...
Once deployed, Railway provides a URL like `capivasign-production.up.railway.app`.

Click the URL in the Railway dashboard to access your CapivaSign instance.
...
If you prefer more control over the setup, deploy CapivaSign manually:
...
- Fork or select the [CapivaSign repository](https://github.com/capivapp/capiva-sign)
...
Click on the CapivaSign service and go to the "Variables" tab. Add the following variables:
...
| `CAPIVASIGN_DISABLE_TELEMETRY`     | Disable anonymous telemetry        | `false` |
...
CapivaSign runs database migrations automatically on startup.
...
- Click on your CapivaSign service
...
Railway provides several options for scaling your CapivaSign deployment:
...
<Steps>
<Step>
### Open your CapivaSign service

In the Railway dashboard, select the CapivaSign service.
...
In the Railway dashboard, select your CapivaSign service and open "Settings".
...
<Callout type="warn">
  CapivaSign stores sessions in the database, so horizontal scaling works without additional
...
Railway uses usage-based pricing. Estimated monthly costs for CapivaSign:
...
CapivaSign requires a signing certificate to sign documents. On Railway, you can:

### Option 1: Use Generated Certificate (Default)

CapivaSign generates a self-signed certificate automatically if none is provided. This works for testing but shows generic certificate information in signed documents.
...
In your CapivaSign service variables, set:
...
<a href="https://app.koyeb.com/deploy?type=git&repository=github.com/capivapp/capiva-sign&branch=main&name=capivasign-app&builder=dockerfile&dockerfile=/docker/Dockerfile">
  <img

...
description: Diagnose and resolve common issues with your self-hosted CapivaSign instance, organized by component.
---
...
```bash
kubectl logs -l app.kubernetes.io/name=capivasign -n capivasign -f
```
...
```bash
# Docker - find errors
docker logs capivasign 2>&1 | grep -i error

# Docker - find warnings
docker logs capivasign 2>&1 | grep -i warn

# Show last 100 lines
docker logs --tail 100 capivasign
```

### Health Checks

CapivaSign exposes a health endpoint to verify the application is running:
...
```bash
docker exec capivasign printenv | grep NEXT
```
...
```bash
docker compose exec capivasign printenv | grep NEXT
```
...
```bash
docker inspect capivasign --format='{{.State.ExitCode}}'
```
...
```bash
# Docker
docker stats capivasign

# Kubernetes
kubectl top pod -n capivasign
```
...
<Steps>
  ...
  <Step>
    Change CapivaSign's port mapping: `-p 3001:3000`
  </Step>
</Steps>
...
```
Error: password authentication failed for user "capivasign"
```
...
```sql
ALTER USER capivasign WITH PASSWORD 'newpassword';
```
...
```
Error: database "capivasign" does not exist
```
...
```
Error: too many connections for role "capivasign"
```
...
```
postgresql://user:password@host:5432/capivasign?connection_limit=5
```
...
```bash
docker exec capivasign printenv | grep SMTP
```
...
```bash
docker exec capivasign nc -zv smtp.example.com 587
```
...
```bash
docker exec capivasign printenv | grep SIGNING
```
...
```bash
docker exec capivasign ls -la /opt/capivasign/cert.p12
```
...
```
Error: EACCES: permission denied, open '/opt/capivasign/cert.p12'
```
...
```
  ...
  <Step>
    Update the certificate file and restart CapivaSign
  </Step>
</Steps>
...
```bash
docker exec capivasign cat /etc/resolv.conf
```
...
- **[GitHub Discussions](https://github.com/capivapp/capiva-sign/discussions)**
- **[Discord](https://documen.so/discord)**: Join the CapivaSign community Discord
- **[GitHub Issues](https://github.com/capivapp/capiva-sign/issues)**: For confirmed bugs, open an issue
...
For security vulnerabilities, do not open a public issue. Contact security@capivapp.com.br directly.
...
<Accordions type="multiple">
  <Accordion title="Check application logs for errors">
    Run `docker logs capivasign` or `docker compose logs capivasign` (or `kubectl logs` for

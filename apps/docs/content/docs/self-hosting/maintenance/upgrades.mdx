---
title: Upgrades
description: Upgrade your self-hosted CapivaSign installation to get the latest features, security patches, and bug fixes.
---
...
CapivaSign follows [Semantic Versioning](https://semver.org/):
...
<Callout type="info">
  Review the [release notes](https://github.com/capivapp/capiva-sign/releases) before upgrading to
...
Check your running CapivaSign version:

<Tabs items={['Docker', 'Docker Compose', 'Kubernetes']}>
<Tab value="Docker">

```bash
docker inspect capivasign --format='{{.Config.Image}}'
```
...
```bash
kubectl get deployment capivasign -n capivasign -o jsonpath='{.spec.template.spec.containers[0].image}'
```
...
```bash
curl -s https://api.github.com/repos/capivasign/capivasign/releases/latest | grep tag_name
```

Or visit the [releases page](https://github.com/capivapp/capiva-sign/releases).
...
```bash
curl -s https://hub.docker.com/v2/repositories/capivasign/capivasign/tags | jq -r '.results[].name'
```
...
```bash
curl -s https://api.github.com/orgs/capivasign/packages/container/capivasign/versions | jq -r '.[].metadata.container.tags[]'
```
...
```bash
docker compose exec database pg_dump -U capivasign capivasign > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl exec -n capivasign deploy/postgres -- pg_dump -U capivasign capivasign > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
pg_dump "postgresql://user:password@host:5432/capivasign" > backup-$(date +%Y%m%d-%H%M%S).sql
```
...
```bash
kubectl get secret capivasign-secrets -n capivasign -o yaml > secrets-backup.yaml
kubectl get configmap capivasign-config -n capivasign -o yaml > configmap-backup.yaml
```
...
```bash
docker pull capivasign/capivasign:1.6.0
```
...
```bash
docker stop capivasign
docker rm capivasign
```
...
docker run -d \
  ...
  -p 3000:3000 \
  --env-file .env \
  -v /path/to/cert.p12:/opt/capivasign/cert.p12:ro \
  capivasign/capivasign:1.6.0
```
...
services:
  capivasign:
    image: capivasign/capivasign:1.6.0
```
...
services:
  capivasign:
    image: capivasign/capivasign:${DOCUMENSO_VERSION:-latest}
```
...
```bash
docker compose ps
docker compose logs -f capivasign
```
...
```bash
kubectl set image deployment/capivasign \
  capivasign=capivasign/capivasign:1.6.0 \
  -n capivasign
```
...
```bash
kubectl rollout status deployment/capivasign -n capivasign
```

Watch pods transition:

```bash
kubectl get pods -n capivasign -w
```
...
```bash
kubectl get pods -n capivasign -o wide
```

Test the health endpoint:

```bash
kubectl port-forward svc/capivasign 3000:80 -n capivasign &
curl http://localhost:3000/api/health
```
...
CapivaSign runs database migrations automatically when the container starts. No manual intervention is required.
...
```bash
docker logs capivasign 2>&1 | grep -i migration
```
...
```bash
kubectl logs -l app.kubernetes.io/name=capivasign -n capivasign | grep -i migration
```
...
# Run migrations only
docker run --rm \
  -e NEXT_PRIVATE_DATABASE_URL="postgresql://user:password@host:5432/capivasign" \
  capivasign/capivasign:1.6.0 \
...
Read the [release notes](https://github.com/capivapp/capiva-sign/releases) carefully.
...
curl -O https://raw.githubusercontent.com/capivasign/capivasign/main/.env.example
...
kubectl rollout history deployment/capivasign -n capivasign

# Rollback to previous version
kubectl rollout undo deployment/capivasign -n capivasign

# Or rollback to a specific revision
kubectl rollout undo deployment/capivasign -n capivasign --to-revision=2
```
...
```bash
docker compose exec -T database psql -U capivasign capivasign < backup-20240115-120000.sql
```
...
```bash
psql "postgresql://user:password@host:5432/capivasign" < backup-20240115-120000.sql
```
...
<Accordions type="multiple">
  <Accordion title="Container fails to start after upgrade">
    **Check logs:** `docker logs capivasign`
  ...
  <Accordion title="Migration errors">
    ...
    **Test connectivity:**
    ...
    ```bash
    docker run --rm postgres:15 psql "postgresql://user:password@host:5432/capivasign" -c "SELECT 1"
    ```
  ...
  <Accordion title="Application errors after upgrade">
    **If the application starts but behaves unexpectedly:**
    ...
    ```bash
    docker logs capivasign 2>&1 | grep -i -E "(warn|error)"
    ```
  </Accordion>

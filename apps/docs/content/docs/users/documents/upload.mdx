...
description: Upload documents to CapivaSign to prepare them for signing. Covers supported formats, file size limits, and upload methods.
---
...
<Callout type="warn">
  CapivaSign does not support password-protected or encrypted PDF files. Remove encryption before
  uploading.
</Callout>

## Supported Formats

CapivaSign accepts two file formats:
...
<Callout type="info">
  ...
  [capivapp.com.br](https://app.capivapp.com.br). Self-hosted instances must
...
![Documents dashboard](/document-signing/capivasign-documents-dashboard.webp)
...
<Steps>
  <Step>
    ...
    Open your [Documents dashboard](https://app.capivapp.com.br/documents).
...
If you do upload a `.docx` file, CapivaSign converts it to PDF before adding it to the envelope. The original `.docx` is discarded. Only the converted PDF is stored, signed, and downloaded.
...
- **Form controls are flattened.** Word content controls (drop-downs, date pickers, checkboxes) become static text or graphics. Use CapivaSign fields for anything that needs to be filled in.
- **Fonts not installed on the server fall back to substitutes.** On capivapp.com.br, common fonts (Calibri, Arial, Times New Roman, etc.) are installed. On self-hosted instances, font fidelity depends on the operator's setup.
...
1. **Processing**: CapivaSign validates the PDF and extracts page information
...
<Accordion title="Only PDF and DOCX files are allowed">
  CapivaSign accepts PDF and DOCX. For other formats (`.doc`, `.odt`, `.rtf`, etc.), export to PDF

...
// app" flow the product will trigger via a capivasign-icp:// deep link.
//
...
const server = createServer(async (req, res) => {
  try {
    ...
    if (req.url === '/api/icp/sign/complete') {
      ...
      const pdf = await PDF.load(baseBytes);
      const fifo = new Fifo({
        certificate: s.chain[0],
        certificateChain: s.chain.slice(1),
        keyType: 'RSA',
        signatureAlgorithm: 'RSASSA-PKCS1-v1_5',
        signatures: [sig],
      });
      const r = await pdf.sign({
        signer: fifo,
        fieldName: 'ICPSig_0',
...
if (lastSealedBytes) {
  try {
    verifyLast(lastSealedBytes);
    ok(true, 'openssl verifies the agent-produced signature on the server PDF');
  } catch (e) {
    ok(false, `openssl verify failed: ${(e.stderr?.toString() || e.message || '').split('\n')[0]}`);
  }
}

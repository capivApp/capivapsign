 ...
 * (production: {@code capivasign-icp://sign?...}). It performs the WHOLE remote
 ...
 *
 * CapivaSign's `complete` then runs the usual server-side notifications, so
 ...
 *   "capivasign-icp://sign?baseUrl=URL&token=RECIPIENT_TOKEN&source=windows-my[&...]"
 *
...
final class CliAgent {
  ...
  static void run(String[] args) throws Exception {
    ...
    if (args[0].startsWith(Brand.PROTOCOL_PREFIX)) {
  ...
  static Map<String, Object> performSign(Map<String, String> opts, Ui ui) throws Exception {
    ...
    try (CertSource source = openSource(opts, ui)) {
      ...
      CapivaSignClient client = new CapivaSignClient(baseUrl, opts.get("bearer"));

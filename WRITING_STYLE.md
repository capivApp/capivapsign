...
This document defines the writing conventions for CapivaSign documentation.
...
1. **Users** - People using the CapivaSign web application to send and sign documents
2. **Developers** - Building integrations with the API or SDKs
3. **Self-hosters** - Running their own CapivaSign instance
...
```typescript
// Basic usage
const document = await capivasign.documents.create({
  title: "Contract",
  file: pdfBuffer,
});

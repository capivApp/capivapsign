 ...
 * CSS nesting (`.capivasign-branded { <user css> }`). The sanitiser itself
 ...
 */
export const SANITIZE_BRANDING_SCOPE_CLASS = 'capivasign-branded';
 ...
 * The sanitised output is later wrapped in `.capivasign-branded { ... }` via
...
const sanitizeRule = (rule: Rule, warnings: SanitizeBrandingCssWarning[]): void => {
  ...
  // the entire sanitised CSS in `.capivasign-branded { ... }` (CSS nesting).

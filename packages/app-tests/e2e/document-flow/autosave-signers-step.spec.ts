...
const addSignerAndSave = async (page: Page) => {
  await page.getByPlaceholder('Email').fill('recipient1@capivapp.com.br');
...
test.describe('AutoSave Signers Step', () => {
  test('should autosave the signers addition', async ({ page }) => {
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients[0].email).toBe('recipient1@capivapp.com.br');
  ...
  test('should autosave the signer update', async ({ page }) => {
    ...
    await page.getByPlaceholder('Name').fill('CapivaSign Manager');
    await page.getByPlaceholder('Email').fill('manager@capivapp.com.br');
    ...
    await expect(async () => {
      ...
      expect(retrievedRecipients.length).toBe(1);
      expect(retrievedRecipients[0].email).toBe('manager@capivapp.com.br');
      expect(retrievedRecipients[0].name).toBe('CapivaSign Manager');
  ...
  test('should autosave the signing order change', async ({ page }) => {
    ...
    await page.getByTestId('signer-email-input').nth(1).fill('recipient2@capivapp.com.br');
    await page.getByLabel('Name').nth(1).fill('Recipient 2');

    await page.getByRole('button', { name: 'Add Signer' }).click();

    await page.getByTestId('signer-email-input').nth(2).fill('recipient3@capivapp.com.br');
    ...
    await expect(async () => {
      ...
      const firstRecipient = retrievedRecipients.find((r) => r.email === 'recipient1@capivapp.com.br');
      const secondRecipient = retrievedRecipients.find((r) => r.email === 'recipient2@capivapp.com.br');
      const thirdRecipient = retrievedRecipients.find((r) => r.email === 'recipient3@capivapp.com.br');
